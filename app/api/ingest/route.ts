...
export async function POST(req: NextRequest) {
  try {
    ...
    // If caseId is not a valid UUID, treat it as a text identifier
    // and store it directly (Supabase will handle conversion or error)
    const { data, error } = await supabase
      .from("messages")
      .insert({ case_id: caseId, source: "upload", topic: text })
      .select()
      .single();
    ...
    if (error) {
      return NextResponse.json(
        {
          ok: false,
          error: `Database error: ${error.message}. If case_id must be UUID, please provide a valid UUID or update the schema to accept text.`,
        },
        { status: 400 }
      );
    }
