...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (Array.isArray(result)) {
      candidates = result as TaskCandidate[];
    } else if (
      result &&
      typeof result === "object" &&
      "task_candidates" in result &&
      Array.isArray(result.task_candidates)
    ) {
      candidates = result.task_candidates as TaskCandidate[];
    } else {
  ...
  const rows = candidates.map((c) => ({
    ...
    status: "pending",
    assignee_agent:
      c.type === "evidence_sort" ? "evidence_sorter" : "client_comms",
    input: {
      messageId,
